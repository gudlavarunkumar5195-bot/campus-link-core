...
serve(async (req) => {
  ...
  try {
    ...
    if (requesterProfile.role !== "admin" || !requesterProfile.school_id) {
      throw new Error("Only school admins can create staff");
    }
    ...
    if (!first_name || !last_name || !email || !employee_id || !position || !hire_date) {
      throw new Error("first_name, last_name, email, employee_id, position and hire_date are required");
    }
    ...
    if (staffErr) throw new Error(`Failed to create staff record: ${staffErr.message}`);
    ...
    console.error("Error in admin-create-staff:", error);
    return new Response(
